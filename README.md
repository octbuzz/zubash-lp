...
1周およそ **1分47秒**。PV 以外は**1枚9秒**。

秒数は `kioskSeconds` プロパティで、書き出し時点の既定値は18秒だが展示には長いので
`tools/kiosk_patch.py` の `SECONDS_PER_PAGE` で9秒に書き換えている（URLからは変えられない）。
変えたいときはこの定数を直してビルドし直すこと。
