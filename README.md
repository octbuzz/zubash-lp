...
`tools/controls_layout.py` で、書き出しの〈左＝コントローラの絵／右＝操作一覧〉という
2カラムを、**ボタンの位置に説明を置く図**へ組み直している。絵と説明が離れていると
どのボタンが何なのか目で追う必要があり、試遊台で一瞬見るには向かないため。
...
X は十字の左、B は右、A は下、Y は上という実際の配置のまま説明を隣に置き、
ボタンと同じ色の短い接続線で結んでいる。START / SELECT は図の下に横帯で残した。

* 説明の英文は100字以内（キオスクの `MAX_CHARS` に引っかからないように）
* ボタンを横一列に並べる都合で、狭い画面では図が画面幅に収まらない。
  カードに `overflow-x: auto` を入れ、**ページごと横スクロールするのではなく
  カードの中でスクロール**させている。キオスク（幅1280）では溢れない
